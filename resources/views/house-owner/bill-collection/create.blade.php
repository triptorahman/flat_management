                        ...
                        <div class="md:col-span-1">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                @if(isset($dueAmount) && $dueAmount > 0)
                                    <div class="flex items-center">
                                        <input type="checkbox" 
                                               name="include_due_amount" 
                                               id="include_due_amount" 
                                               value="1"
                                               {{ old('include_due_amount', true) ? 'checked' : '' }}
                                               onchange="toggleDueAmount()"
                                               class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                        <label for="include_due_amount" class="ml-2 text-sm text-gray-600 cursor-pointer">
                                            Include in bill
                                        </label>
                                    </div>
                                @endif
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" 
                                       ...
                                       placeholder="0.00"
                                       readonly
                                       class="pl-7 mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-50 text-gray-700 cursor-not-allowed focus:border-gray-300 focus:ring-gray-300 {{ (isset($dueAmount) && $dueAmount > 0) ? 'bg-red-50 border-red-200' : '' }}">
                            </div>
                            @error('due_amount')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500">
                                @if(isset($dueAmount) && $dueAmount > 0)
                                    ...
                                    <br>
                                    <span class="text-blue-600">Use checkbox above to include/exclude from current bill</span>
                                @else
...
<script>
    // Calculate due amount for the pre-selected flat
    async function calculateDueAmount() {
        ...
        try {
            // Due amount is already calculated by the controller
            // Just highlight the field if there's an outstanding balance
    ...
    // Toggle due amount inclusion in bill
    function toggleDueAmount() {
        const checkbox = document.getElementById('include_due_amount');
        const dueAmountInput = document.getElementById('due_amount');
        const originalAmount = parseFloat(dueAmountInput.getAttribute('data-original-amount') || dueAmountInput.value) || 0;
        
        // Store original amount if not already stored
        if (!dueAmountInput.hasAttribute('data-original-amount')) {
            dueAmountInput.setAttribute('data-original-amount', originalAmount);
        }
        
        if (checkbox && checkbox.checked) {
            // Include due amount - restore original value
            dueAmountInput.value = originalAmount.toFixed(2);
            dueAmountInput.classList.add('bg-red-50', 'border-red-200');
            dueAmountInput.classList.remove('bg-gray-50', 'border-gray-300');
        } else {
            // Exclude due amount - set to 0
            dueAmountInput.value = '0.00';
            dueAmountInput.classList.remove('bg-red-50', 'border-red-200');
            dueAmountInput.classList.add('bg-gray-50', 'border-gray-300');
        }
    ...
    function updateBillSummary() {
        ...
        // Get due amount - only if checkbox is checked (when it exists)
        let dueAmount = 0;
        const dueAmountInput = document.getElementById('due_amount');
        const includeDueCheckbox = document.getElementById('include_due_amount');
        
        if (includeDueCheckbox) {
            // If checkbox exists, only include due amount if checked
            if (includeDueCheckbox.checked) {
                dueAmount = parseFloat(dueAmountInput.value) || 0;
            }
        } else {
            // If no checkbox, include the due amount as is
            dueAmount = parseFloat(dueAmountInput.value) || 0;
        }
        ...
        if (dueAmount > 0) {
            ...
            dueAmountDisplay.classList.remove('text-gray-900');
            dueAmountDisplay.classList.add('text-red-600');
        } else {
            totalDisplay.classList.remove('text-red-600');
            totalDisplay.classList.add('text-blue-600');
            dueAmountDisplay.classList.remove('text-red-600');
            dueAmountDisplay.classList.add('text-gray-900');
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize due amount checkbox state
        const dueAmountInput = document.getElementById('due_amount');
        const includeDueCheckbox = document.getElementById('include_due_amount');
        
        if (dueAmountInput && includeDueCheckbox) {
            // Store original amount
            const originalAmount = parseFloat(dueAmountInput.value) || 0;
            dueAmountInput.setAttribute('data-original-amount', originalAmount);
        ...
        if (includeDueCheckbox) {
            includeDueCheckbox.addEventListener('change', toggleDueAmount);
        }
    });
</script>
@endsection
